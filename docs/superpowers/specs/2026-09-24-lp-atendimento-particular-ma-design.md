...
src/
  ...
  components/                Topbar, CtaWhatsApp, Secao, Foto, Accordion, Abas, CartaoLocal, AvisoCookies
...
### 5.9 Botão flutuante
Removido a pedido do André (24/09/2026, feat/visual bd8b60f; confirmado por ele ao Maestro em 25/09/2026). Não há botão de WhatsApp fixo; a conversão vem dos CTAs das seções.

### 5.10 Aviso de cookies
Barra inferior não bloqueante com "Aceitar" e "Recusar" e link para a política. Padrão negado até a escolha. Não pode cobrir o CTA do hero (decisão do Maestro, 25/09/2026, delegada pelo André; achado do Dev a 360x740 e parecer R31 a 320x568). Regra reativa, enquanto não há escolha: se a barra não se sobrepõe ao CTA do hero na posição atual (desktop), ela aparece; se se sobrepõe ou pode se sobrepor, ela fica oculta enquanto o CTA do hero está na viewport ou abaixo dela e aparece quando o CTA passou para cima da viewport; se a pessoa volta ao hero sem ter escolhido, a barra se oculta de novo. Carga direta em `/<slug>` (CTA acima da viewport) mostra a barra na hora. O botão do rodapé abre a barra na hora, em qualquer caso. A escolha fica em `localStorage` (try/catch) e pode ser mudada pelo link do rodapé. Clique em CTA não é consentimento.
...
- **Carregamento do GTM:** depois do primeiro frame, no que vier primeiro: `requestIdleCallback` com `{ timeout: 1500 }` (fallback `setTimeout` de 1,5 s) ou a primeira interação (`pointerdown`, `touchstart`, `keydown` ou `scroll`, passivos, uma vez). Se o usuário clicar num CTA antes, o clique dispara o carregamento na hora. A navegação não depende do `eventCallback` do GTM (a validação mostrou que ele volta antes de as bibliotecas do GA4 e do Ads enviarem, mesmo com o GTM já carregado): ela acontece 150 ms depois de um `PerformanceObserver` ver a requisição da conversão do Ads (ID 18460652540) iniciada depois do clique, ou no teto de 1.500 ms se o GTM estava pronto no clique e de 4.000 ms se não estava (subido de 3.000 depois da T13, em que o clique antes da hidratação com consentimento recusado passou com só 8 ms de folga), o que vier primeiro; durante a espera o link fica com `aria-busy` e aparência de pressionado. Cliques antes da hidratação são segurados por um script inline mínimo no `index.html` (captura em `a[href*="wa.me/"]`, navegação de segurança em 4.500 ms) e processados pela app ao hidratar, com o teto contado do clique original. O contêiner tem a tag "Ads - Google tag" (AW-18460652540) na Inicialização, para a biblioteca do Ads carregar antes do clique.
- **Endereço sem termo de busca:** na carga, antes de pôr `pagina_limpa` e antes do GTM, a página tira da barra de endereço (`history.replaceState`) o `utm_term` e qualquer `utm_*` fora da convenção fechada, mantendo caminho, `gclid`, `gbraid`, `wbraid`, `gad_source`, as UTMs válidas e os demais parâmetros. Motivo (validação da parte B): a tag de conversão do Ads e o `ccm/collect` enviam a URL completa da barra de endereço. Regra para a Effect: nenhum modelo de acompanhamento ou sufixo de URL final usa `{keyword}` nem `utm_term`. **Decisão do André (25/09/2026): as campanhas não usam UTMs;** a atribuição do Google Ads vem só da marcação automática (`gclid`). O código continua aceitando as UTMs da convenção se aparecerem, sem efeito quando ausentes. Eventos anteriores ficam na fila do `dataLayer`, que o GTM processa ao carregar.
...
|---|---|---|
| `clique_whatsapp` (conversão primária) | `local_cta` (topbar, hero, identificacao, autoavaliacao, como_funciona, sobre, onde_atende, faq, rodape), `cidade`, `local`, `utm_source`, `utm_medium`, `utm_campaign`, `utm_content` (sem `gclid`) | Todo CTA de agendamento |
...
- **Browser/QA (390x844, 820x1180, 1440x900):** topbar, CTAs, "Veja onde ele atende", autoavaliação (toque, teclado, leitor de tela básico), seletor de cidade, as 11 abas, os 14 mapas, os 14 "Como chegar", accordion, ausência de botão fixo de WhatsApp, aviso de cookies.
...
1. **Resolvido (25/09/2026):** o destaque "Clínicas" do Instagram do médico confirma os 14 endereços da página. Mendesclin: Praça do Mercado Central, nº 14 (a ficha do Maps diz R. Gonçalves Dias; o mapa usa a ficha).
2. **Resolvido:** Clinimed (Rua 28 de Julho, Centro) e CM LAB Graça Aranha (Rua São Francisco, S/N) não têm número também no Instagram; sem ficha no Maps, o mapa mostra a rua.
3. **Resolvido:** Presidente Dutra é a Clínica Levive (Rua 28 de Junho Sul, nº 583B), confirmada no Instagram.
4. Dias de atendimento por cidade: ausentes. **Decisão do André (25/09/2026): seguir sem.**
5. Valor, pagamento, duração e retorno: ausentes. **Seguir sem.**
6. Graduação, residência e experiência: ausentes. **Seguir sem.**
7. Arquivo do logo (monograma): não recebido.
8. **Decisão do André (25/09/2026):** não usar "Instituto" em nenhum lugar; a marca é só "Dr. Patrick Santos" (o build e os testes já bloqueiam o termo).
...
14. **Domínio definitivo: drpatricksantos.com.br** (domínio principal, aprovado pelo André em 25/09/2026). DNS na Hostinger: @ e www em A 76.76.21.21 (Vercel), backup da zona em .maestri/; www e lp-dr-santos.vercel.app redirecionam (301, mantendo caminho e query) para o domínio principal, configurado no painel da Vercel (Settings > Domains) pelo Maestro no Chrome do André; SITE_URL e canonical no novo domínio. o `SITE_URL` e o canonical apontam para `lp-dr-santos.vercel.app`. Se a campanha usar outro domínio, atualizar o `SITE_URL`, conferir o HTML servido e a URL canônica escolhida no Search Console, e testar os sitelinks (`/onde-atende` e `/onde-atende/`, com query) com status 200.
15. **Aviso de cookies adiado no celular (parecer R31):** quem toca no CTA do hero sem rolar não vê a primeira camada do aviso, embora o modo avançado já envie sinais limitados sem cookies com consentimento negado; a política explica isso e o link está no rodapé. Levar à revisão jurídica formal (item 10) e avaliar um acesso discreto à política na primeira dobra que não cubra o CTA.
16. **Riscos residuais de LGPD (pareceres R36 e R37), para a revisão jurídica:** base legal dos sinais sem cookies do modo avançado (legítimo interesse, art. 7º, IX, escolhido pelo André; a ANPD não aceita legítimo interesse para dado sensível, e a visita a uma LP médica pode ser inferência de saúde); contratos, países, mecanismo de transferência e retenção reais de Google, Meta e Vercel; dispensa de encarregado (Resolução CD/ANPD 2/2022) ou publicação do contato; mapas do Google carregados sem consentimento (decisão do André; a política informa); aviso curto de medição tirado do hero e levado ao rodapé como links (decisão do André, 25/09/2026), o que enfraquece a informação antes do primeiro clique no modo avançado; valores de parâmetros com nome permitido (gclid e afins) não são verificados como opacos; papéis das clínicas no agendamento.
...
- **LGPD, parecer R36 (decisões do André, 25/09/2026):** (1) Consent Mode continua avançado, com aviso curto visível abaixo do CTA do hero ("O Google mede esta visita, mesmo sem cookies." + link Privacidade) e a primeira camada do aviso e a política explicando os sinais técnicos antes da escolha; validação jurídica formal fica pendente. (2) Duas categorias de cookies (Medição de visitas: `analytics_storage`; Medição de anúncios: `ad_storage` e `ad_user_data`), desligadas por padrão, com Aceitar tudo, Recusar tudo e Escolher; `ad_personalization` sempre negado; escolha guardada com versão e data; revogação apaga os cookies de medição próprios. (3) Mapas do Google: **sempre visíveis** (nova decisão do André, 25/09/2026: "sempre deixe os embeds do maps visíveis em 100% do tempo, não quero clicar pra poder ver"); carregam ao chegar na seção, com ou sem aceite, e a política informa que o Google recebe dados técnicos ao carregar. (4) Quem atende o WhatsApp é o próprio Dr. Patrick Santos, que marca nas clínicas e informa a elas só os dados do agendamento (nome e telefone); o resumo da dor fica com o médico e exige consentimento específico (art. 11, I). Endereço: antes do GTM ficam só gclid, gbraid, wbraid, gad_source, gad_campaignid (só dígitos, parecer R38), a cidade validada e UTMs válidas. Textos em `.maestri/textos-lgpd.md`. Critério do aviso curto (decisão do Maestro sobre o R38): ele fica imediatamente antes do CTA do hero na ordem da página, com 16 px, e aparece inteiro na primeira dobra junto do CTA a 360x740 e 390x844; numa rolagem normal a pessoa passa por ele antes de alcançar o CTA. Não é fixado na tela, para não competir com o CTA.
- **Rodapé e termos de uso (decisão do André, 25/09/2026):** "mover o 'O Google mede esta visita, mesmo sem cookies. Privacidade' para o rodapé [...] e em vez dessa frase deixar apenas o botão de política de privacidade e termos de uso". O aviso curto sai do hero; o rodapé tem os links "Política de privacidade" e "Termos de uso" (página nova /termos-de-uso.html, textos em .maestri/termos-de-uso.md). Isso substitui o critério do aviso antes do CTA definido depois do R38; a informação prévia ao primeiro clique fica no aviso de cookies e nos links do rodapé (risco residual no §17, item 16).
